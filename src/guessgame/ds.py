...
def load_syn_data_latest(ds_path: str):
    ...
    return MedOnc(
        pd.read_parquet(ds_path + "/synth/medonc/mare/tables/patients.pq/" + ts + "/patients.pq"),
        pd.read_parquet(ds_path + "/synth/medonc/mare/tables/lines.pq/" + ts + "/lines.pq"),
        pd.read_parquet(ds_path + "/synth/medonc/mare/tables/updates.pq/" + ts + "/updates.pq"),
        pd.read_parquet(ds_path + "/synth/medonc/mare/tables/medicine.pq/" + ts + "/medicine.pq"),
    )
            ...
            for j, (uid, u) in enumerate(updates[updates['line_id'] == lid].iterrows()):
                ...
                for k, (mid, m) in enumerate(medicine[medicine['update_id'] == uid].iterrows()):
                    ...
                    if isinstance(notes, str):
                        ...
                        if match:
                            pfuns = {
                                'fmgm2': lambda d, acc: round(d * bsa / acc) * acc if bsa else 'INV: no BSA',
                                'fmgkg': lambda d, acc: round(d * weight / acc) * acc if weight else 'INV: no weight',
